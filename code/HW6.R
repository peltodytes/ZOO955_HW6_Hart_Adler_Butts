...
###=================== Randomly selecting zooplankton or macroinvertebrates ###===================
# initial populations
zoop = 100000
macinv = 700
yp = 1
...
for (i in 2: nrow(df)){
  ...
  for (j in 1:200){
    ...
    bm = bm + ifelse(food == 'macinv', 100, 1)
    
    # when yellow perch consume X bm, they are done for the day and the inner loop ends. Represents satiation. 
    if  (bm >= 300 ) break
...
for (i in 2: nrow(df)){
  ...
  for (j in 1:200){
    ...
    # sampling one individual from the populations of zoops & macinvs
    food = sample(c('macinv', 'zoop'), size = 1, prob = c(.60,.40))
    ...
    # Add in a satiation term, so when Yellow perch eat 300 individuals they feel satiated
    if  (bm >= 300 ) break
